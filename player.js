class Player extends Enemy {
  ...
  usePotion(type) {
    if (type === "health" && this.healthPotions.length > 0) {
      ...
      game.battleText = [];
      game.addToBattleText(
        `❤️ ${game.player.name} the ${game.player.type}'s health was restored to ${game.player.healthPts}HP`
      );
      game.addToBattleText(
        `❤️ You have ${this.healthPotions.length} remaining`
      );
